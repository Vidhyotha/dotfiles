...
- `~/.local/bin/bt-hfp-watcher.sh` + `~/.local/share/systemd/user/bt-hfp-watcher.service` — event-driven HFP workaround. The PipeWire/WirePlumber BT transport re-engage bug makes the **second** HFP call fail until WirePlumber is restarted (btmon-proven: HCI/eSCO is clean, it's a stale `spa.bluez5` transport). The watcher subscribes to PipeWire node-state events via `pw-mon` (polling-free) and restarts WirePlumber a few seconds after every call ends so the next call is always clean. `SUPER + B` triggers the same restart manually as a fallback.
